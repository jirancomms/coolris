    ...
    $.fn.imageMaps = function(options) {
        ...
        this.each(function(i, item){
            ...
            $(options.addBtn+':eq('+ i +')').unbind('click').on('click', function(){
                ...
                let oDiv   = $(`<div id="imagemaps-rect-${i}-${index}" style="width:${(coords[2]-coords[0]) || options.rectWidth}px;height:${(coords[3]-coords[1]) || options.rectHeight}px;position:absolute;left:${coords[0] || 0}px;top:${coords[1] || 0}px;background-color:rgba(255,0,255,0.5)" data-index="${index}">${count}</div>`);
                _this.append(oDiv);
